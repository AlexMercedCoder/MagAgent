...
def test_generate_preview_save_inspect_and_conflict(tmp_path: Path) -> None:
    ...
    conflict = save_graph(document, "work.agraph.yaml", project=tmp_path, expected_digest=saved["digest"])
...
def test_all_node_templates_are_strictly_valid() -> None:
    for kind in ("task", "decision", "gate", "loop", "map", "subgraph"):
        document = {
            "ags_version": "1.0", "kind": "AgenticGraph", "id": f"test/{kind}",
            "title": kind, "objective": "Test template", "entrypoints": [kind],
...
def test_rename_and_duplicate_update_references() -> None:
    document = {
        "ags_version": "1.0", "kind": "AgenticGraph", "id": "test/rename",
        "title": "Rename", "objective": "Rename", "entrypoints": ["a"],
        "nodes": {
            "a": {**node_template("task"), "outputs": {"summary": {"type": "markdown", "description": "Result"}}},
            "b": {
                **node_template("task"), "depends_on": ["a"],
                "inputs": {"result": {"type": "markdown", "description": "Result", "from": "nodes.a.outputs.summary"}},
            },
        },
    }
    ...
    assert renamed["document"]["nodes"]["b"]["inputs"]["result"]["from"] == "nodes.inspect.outputs.summary"
...
def test_graph_authoring_cli_round_trip(tmp_path: Path) -> None:
    ...
    preview = runner.invoke(app, ["graph", "preview", "--input", "-", "--project", str(tmp_path)], input=json.dumps(document))
    assert preview.exit_code == 0
    target = tmp_path / "roundtrip.agraph.json"
    applied = runner.invoke(app, ["graph", "apply", str(target), "--input", "-", "--project", str(tmp_path)], input=json.dumps(document))
    ...
    renamed = runner.invoke(app, ["graph", "rename-node", "inspect", "audit", "--input", "-"], input=json.dumps(document))
