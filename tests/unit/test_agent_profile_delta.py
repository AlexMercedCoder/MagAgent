...
from magent.agent_profiles.delta import apply_delta, make_delta, restore_checkpoint
...
def test_delta_writes_state_and_bumps_revision_exactly_once(tmp_path: Path) -> None:
    ...
    delta = make_delta(profile, [{"op": "add", "path": "/state/-", "value": {"id": "style", "content": "Skip formatter findings"}}])
...
def test_delta_secret_is_scrubbed_before_disk(tmp_path: Path) -> None:
    path, profile = _profile(tmp_path)
    delta = make_delta(profile, [{"op": "add", "path": "/state/-", "value": {"id": "bad", "content": "key sk-abcdefghijklmnop1234"}}])
...
def test_stale_revision_is_rejected_without_modifying_file(tmp_path: Path) -> None:
    path, profile = _profile(tmp_path)
    delta = make_delta(profile, [{"op": "add", "path": "/state/-", "value": {"id": "a", "content": "A"}}])
    first = make_delta(profile, [{"op": "add", "path": "/state/-", "value": {"id": "b", "content": "B"}}])
