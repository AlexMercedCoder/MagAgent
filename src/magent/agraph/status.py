...
def graph_status(
    store: WorkbenchStore, run_id: str, *, event_limit: int = 500
) -> dict[str, Any] | None:
    record = next(
    ...
    if root is None:
        root = next(
            (
                task
                for task in runtime.list_tasks(limit=1000)
                if task.get("kind") == "agentic_graph"
                and (task.get("metadata") or {}).get("run_id") == run_id
            ),
            None,
        )
    ...
    child_by_scope = {
        str(
            (task.get("metadata") or {}).get("scope_path")
            or (task.get("metadata") or {}).get("graph_node_id")
            or (task.get("metadata") or {}).get("node_id")
            or ""
        ): task
    ...
    for scope_path, item in node_records.items():
        if "." not in scope_path and "[" not in scope_path:
            ...
            scope["nodes"][scope_path] = {
                "status": statuses[scope_path],
    ...
    for node_id, definition in (document.get("nodes") or {}).items():
        ...
        blocked = (
            blocking_reasons(document, node_id, statuses, scope)
            if state in {"pending", "ready", "queued", "waiting", "blocked"}
            else []
        )
        nodes.append(
            {
                ...
                "state": state,
                "summary": str(
                    (task or {}).get("final_audit", {}).get("summary")
                    or record_item.get("x-magagent-summary")
                    or ""
                ),
                "error_code": str(
                ...
                "blocked_by": blocked
                or list(
                    (task or {}).get("metadata", {}).get("blocked_by")
                    or record_item.get("blocked_by")
                    or []
                ),
                "task": task,
                "record": record_item,
    ...
    return {
        ...
        "summary": (record.get("metadata") or {}).get("x-magagent-summary")
        or (root or {}).get("final_audit")
        or {},
        "run": record,
        "task": root,
        "nodes": nodes,
