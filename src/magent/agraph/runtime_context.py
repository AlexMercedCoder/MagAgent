...
async def authorize_graph_tool(tool_name: str, args: dict[str, Any], cwd: str) -> dict[str, Any]:
    ...
    if tool_name not in policy.allowed_tools:
        return {"ok": False, "error": f"RT012 tool {tool_name!r} was not declared by this graph node"}
    if policy.observer:
        error = policy.observer(tool_name, args)
        if error:
            return {"ok": False, "error": error}
    permission_error = _permission_error(policy.permissions, tool_name, args, cwd)
    if permission_error:
        return {"ok": False, "error": f"RT012 {permission_error}"}
    mutating = tool_name in MUTATING_TOOLS or (tool_name == "run_shell" and _shell_may_mutate(str(args.get("command", ""))))
    if mutating and policy.approval and not policy.approved:
        if not await policy.approval(policy.approval_prompt, {"tool": tool_name, "args": args}):
            return {"ok": False, "error": "RT015 side-effect checkpoint rejected"}
        policy.approved = True
    return {"ok": True}


def _permission_error(permissions: tuple[str, ...], tool_name: str, args: dict[str, Any], cwd: str) -> str:
    if not permissions:
        return "node declares no permissions"
    if tool_name in {"read_file", "read_file_range", "outline_file", "list_dir", "search_codebase", "read_image", "diff_files"}:
        return "" if _path_allowed(permissions, "fs:read:", str(args.get("path", args.get("path_a", "."))), cwd) else "filesystem read exceeds the requested permission ceiling"
    if tool_name in MUTATING_TOOLS and any(key in args for key in ("path", "file_path")):
        raw = str(args.get("path", args.get("file_path", "")))
        return "" if _path_allowed(permissions, "fs:write:", raw, cwd) else "filesystem write exceeds the requested permission ceiling"
    if tool_name == "run_shell":
        command = str(args.get("command", ""))
        patterns = [item.removeprefix("shell:exec:") for item in permissions if item.startswith("shell:exec:")]
        return "" if any(fnmatch.fnmatch(command, pattern) for pattern in patterns) else "shell command exceeds the requested permission ceiling"
    if tool_name.startswith(("web_", "http_", "browser_")):
        return "" if any(item.startswith(("net:", "web:")) for item in permissions) else "network access exceeds the requested permission ceiling"
    return ""


def _path_allowed(permissions: tuple[str, ...], prefix: str, raw: str, cwd: str) -> bool:
    root = Path(cwd).resolve()
    path = (root / raw).resolve(strict=False) if not Path(raw).is_absolute() else Path(raw).resolve(strict=False)
...
def _shell_may_mutate(command: str) -> bool:
    ...
    return any(token in lowered for token in (" >", " >>", " rm ", " mv ", " cp ", " install ", " commit ", " push ", " apply "))
