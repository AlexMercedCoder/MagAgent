...
def profile_contract(project: str | Path = ".", config: Any | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "choices": {
            ...
            "profiles": [profiles[name].as_dict(include_document=False) for name in sorted(profiles)],
            "providers": [
                {
                    "id": name,
...
def apply_profile(
    ...
    *,
    ...
    if target.exists():
        ...
        candidate["proposals"] = copy.deepcopy(current_document.get("proposals", []))
...
def profile_checkpoints(
    ...
    if resolved is None:
        return {"ok": False, "contract": PROFILE_CONTRACT, "error": f"Agent profile not found: {name}"}
    ...
    for candidate in sorted(directory.glob(f"{resolved.name}-r*-*{path.suffix}.bak"), reverse=True):
        try:
            ...
            continue
    return {"ok": True, "contract": PROFILE_CONTRACT, "profile": resolved.name, "checkpoints": checkpoints}
...
def rollback_profile(
    ...
    *,
    ...
    if not expected_digest or expected_digest != resolved.profile_digest:
        return {"ok": False, "contract": PROFILE_CONTRACT, "conflict": True, "error": "Profile changed or no profile_digest was supplied.", "current_digest": resolved.profile_digest}
    allowed = {item["path"] for item in profile_checkpoints(name, project=project, config=config)["checkpoints"]}
    target = str(Path(checkpoint).expanduser().resolve())
    if target not in allowed:
        return {"ok": False, "contract": PROFILE_CONTRACT, "error": "Checkpoint is not part of this profile's revision history."}
...
def clone_profile(
    ...
    *,
    ...
    if resolved is None:
        return {"ok": False, "contract": PROFILE_CONTRACT, "error": f"Agent profile not found: {source}"}
...
def delete_profile(
    ...
    *,
    ...
    if resolved is None:
        return {"ok": False, "contract": PROFILE_CONTRACT, "error": f"Agent profile not found: {name}"}
    if resolved.source_path is None:
        return {"ok": False, "contract": PROFILE_CONTRACT, "error": "Managed profiles cannot be deleted."}
    if not expected_digest or expected_digest != resolved.profile_digest:
        return {
            "ok": False,
            "contract": PROFILE_CONTRACT,
    ...
    return {"ok": True, "contract": PROFILE_CONTRACT, "profile": resolved.name, "deleted": str(resolved.source_path)}
...
def _dependencies(
    ...
    missing_tools = sorted(
        name for name in requested_tools if name not in aliases and "*" not in name and name not in tool_names
    )
    missing = {
        "tools": missing_tools,
...
def _templates() -> list[dict[str, Any]]:
    return [
        {"id": "general", "title": "General assistant", "description": "Coding and productivity with balanced permissions.", "tools": ["*"], "network": "read"},
        {"id": "coder", "title": "Coding specialist", "description": "Inspect, edit, test, and review project code.", "tools": ["read", "write", "edit", "search", "shell"], "network": "read"},
        {"id": "researcher", "title": "Research specialist", "description": "Web research and source synthesis without project writes.", "tools": ["read", "web"], "network": "read"},
        {"id": "reviewer", "title": "Review specialist", "description": "Read-only code and documentation review.", "tools": ["read", "search"], "network": "none"},
        {"id": "custom", "title": "Custom", "description": "Start with explicit choices.", "tools": [], "network": "none"},
    ]
